...
此外，上能帶（$S = +$）受長程交換交互作用驅動，在次波長大動量區域下呈現線性色散關係 $E^X_{+,\mathbf{Q}} \approx E^X_0 + 2\gamma Q$。由於其色散關係在動量空間中具有與光子色散相同的線性特性，該模式常被定義為「類光激子（Light-like exciton）」。此線性色散曲線與邊緣電漿子色散曲線之共振交點，確立了動量空間中的相位匹配條件，此即本研究進行「動量工程（Momentum Engineering）」之物理基礎，相關共振特性將於第四章詳述。
